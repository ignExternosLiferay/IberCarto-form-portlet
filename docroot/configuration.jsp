...
boolean sendMailUser = GetterUtil.getBoolean(portletPreferences.getValue("sendMailUser", StringPool.BLANK));
	...
	<liferay-ui:panel-container extended="<%= Boolean.TRUE %>" id="webFormConfiguration" persistState="<%= true %>">
		<liferay-ui:panel collapsible="<%= true %>" extended="<%= true %>" id="webFormGeneral" persistState="<%= true %>" title="form-information">
			<aui:fieldset>
				...
				<%-- Se sustituye este captcha por el de Google
				...
				<aui:field-wrapper cssClass="lfr-input-text-container" label="text-on-success">
					<liferay-ui:input-localized name="preferences--successTxt--" xml="<%= HtmlUtil.toInputSafe(successTxt) %>" />
				</aui:field-wrapper>
				<%-- <aui:input label="text-on-success" name="preferences--successTxt--" value="<%= HtmlUtil.toInputSafe(successTxt) %>" wrapperCssClass="lfr-input-text-container" /> --%>
				<aui:input label="send-mail-user" name="preferences--sendMailUser--" type="checkbox" value="<%= sendMailUser %>" />
			</aui:fieldset>
		</liferay-ui:panel>
